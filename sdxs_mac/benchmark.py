...
#     parser.add_argument("--device", type=str, default="mps", choices=["cpu", "cuda", "mps"], help="Устройство для генерации (cpu, cuda, mps).")
...
def benchmark(pipe, prompts, device="mps"):
    ...
    for prompt in prompts:
        try:
            ...
            # Запись общей потребляемой памяти системы
            ...
            results.append({
                ...
                "Total Memory Used (MB)": round(total_memory, 2),
                "Iterations/s": round(1 / generation_time, 2) if generation_time > 0 else 0
            })
            ...
            results.append({
                "Prompt": prompt,
                "Generation Time (s)": "Error",
                "Memory Before (MB)": "Error",
                "Memory After (MB)": "Error",
                "Total Memory Used (MB)": "Error",
                "Iterations/s": "Error"
            })
